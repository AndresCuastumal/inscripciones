  ...
  <div class="container mt-5">
    <div class="row">
      <div class="bg-light text-info ">
        <h3>Consultar establecimiento o propietario</h3>
        <p class="text-justify" >El sistema consultará en su base de datos si el establecimiento y/o proietario que está solicitando una nueva inscripción ya están registrados. Si no se encuentran registrados, se solicitarán los datos del propietario y del establecimiento respectivamente.</p>      
      </div>    
    </div>
    <div>
    <br></br>  
      <form method ="POST">
        ...
        <!-- *** ESTA DIV ESTÁ DESHABILITADO PORQUE SE REALIZARÁ UNA CONSULTA GENERAL ***

        <div class="mb-3">          
          <select id="selectedOption" name="opciones" class="mb-3 form-control bg-primary text-white" required>
              <option value= "" disabled selection selected>Seleccione tipo de consulta</option>
              <option value="Doc_establecimiento">Consulta por NIT del establecimiento</option>
              <option value="Doc_propietario">Consulta por documento del propietario</option>
              <option value="Nom_establecimiento">Consulta por nombre del establecimiento</option>
              <option value="Nom_propietario">Consulta por nombre del propietario</option>
              <option value="rango_fecha">Rango fecha de registro</option>           
          </select>
        </div>-->        
        <div class="mb-3" id="input-container">
        <!-- *** ESTE LABEL SE DESHABILITA Y ES EL QUE ACTUA CON EL SCRIPT No 1 *** 
        <label for="doc" id="dynamicLabel" class= "hidden" ></label> -->  
        ...
        <table class="table">
          <thead>
            ...
            <tr>
              ...
              <th class="text-primary">Nombre Propietario</th> <th class="text-primary">Doc Propietario</th><th class="border-end text-primary">Imprimir inscripción</th>
              <th class="text-primary"> editar</th> <th class="text-primary"> Solicitar visita</th> <th class="text-primary">Nuevo</th>
            </tr>
          </thead>
          <tbody>
            <?php
            ...
            <tr>
            <td>
              <p>No se encuentra el establecimiento o representante legal consultado? Realice un nuevo registro:</p>
            </td>
            <td>
...
<!-- *** SCRIPT No 1 ***
PARA CARGAR UN LABEL DESPUÉS DE HABER SELECCIONADO UN ELEMENTO DE SELECT LIST
-->

<!-- <script>
  const selectElement = document.getElementById('selectedOption');
  const labelElement = document.getElementById('dynamicLabel');

  selectElement.addEventListener('change', function () {
    const selectedOption = selectElement.value;

    // Mostrar el label si está oculto
    labelElement.classList.remove('hidden');

    // Cambiar el texto del label según la opción seleccionada
    switch (selectedOption) {
        case 'Doc_establecimiento':
            labelElement.textContent = 'Ingrese el nit del establecimiento sin dígito de verificación:';
            break;
        case 'Doc_propietario':
            labelElement.textContent = 'Ingrese el número de documento de identidad del propietario:';
            break;
        case 'Nom_establecimiento':
        labelElement.textContent = 'Ingrese el nombre del establecimiento:';
            break;
        case 'Nom_propietario':
        labelElement.textContent = 'Ingrese el nombre del propietario:';
            break;
        case 'rango_fecha':
        labelElement.textContent = 'Ingrese el rango de fechas de la posible inscripción';
            break;                  
        default:
            labelElement.textContent = 'Selecciona una opción válida';
    }
  });
</script> -->
